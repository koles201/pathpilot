// themePrimitives.ts
import { alpha, PaletteMode } from "@mui/material/styles"

// Цветовые палитры — только plain объекты, без функций и createTheme
export const brand = {
  50: "hsl(210, 100%, 95%)",
  100: "hsl(210, 100%, 92%)",
  200: "hsl(210, 100%, 80%)",
  300: "hsl(210, 100%, 65%)",
  400: "hsl(210, 98%, 48%)",
  500: "hsl(210, 98%, 42%)",
  600: "hsl(210, 98%, 55%)",
  700: "hsl(210, 100%, 35%)",
  800: "hsl(210, 100%, 16%)",
  900: "hsl(210, 100%, 21%)",
}

export const gray = {
  50: "hsl(220, 35%, 97%)",
  100: "hsl(220, 30%, 94%)",
  200: "hsl(220, 20%, 88%)",
  300: "hsl(220, 20%, 80%)",
  400: "hsl(220, 20%, 65%)",
  500: "hsl(220, 20%, 42%)",
  600: "hsl(220, 20%, 35%)",
  700: "hsl(220, 20%, 25%)",
  800: "hsl(220, 30%, 6%)",
  900: "hsl(220, 35%, 3%)",
}

export const green = {
  50: "hsl(120, 80%, 98%)",
  100: "hsl(120, 75%, 94%)",
  200: "hsl(120, 75%, 87%)",
  300: "hsl(120, 61%, 77%)",
  400: "hsl(120, 44%, 53%)",
  500: "hsl(120, 59%, 30%)",
  600: "hsl(120, 70%, 25%)",
  700: "hsl(120, 75%, 16%)",
  800: "hsl(120, 84%, 10%)",
  900: "hsl(120, 87%, 6%)",
}

export const orange = {
  50: "hsl(45, 100%, 97%)",
  100: "hsl(45, 92%, 90%)",
  200: "hsl(45, 94%, 80%)",
  300: "hsl(45, 90%, 65%)",
  400: "hsl(45, 90%, 40%)",
  500: "hsl(45, 90%, 35%)",
  600: "hsl(45, 91%, 25%)",
  700: "hsl(45, 94%, 20%)",
  800: "hsl(45, 95%, 16%)",
  900: "hsl(45, 93%, 12%)",
}

export const red = {
  50: "hsl(0, 100%, 97%)",
  100: "hsl(0, 92%, 90%)",
  200: "hsl(0, 94%, 80%)",
  300: "hsl(0, 90%, 65%)",
  400: "hsl(0, 90%, 40%)",
  500: "hsl(0, 90%, 30%)",
  600: "hsl(0, 91%, 25%)",
  700: "hsl(0, 94%, 18%)",
  800: "hsl(0, 95%, 12%)",
  900: "hsl(0, 93%, 6%)",
}

// Простая типографика — размеры в rem, никакого createTheme
export const typography = {
  fontFamily: "Inter, sans-serif",
  h1: { fontSize: "3rem", fontWeight: 600, lineHeight: 1.2, letterSpacing: "-0.5px" },
  h2: { fontSize: "2.25rem", fontWeight: 600, lineHeight: 1.2 },
  h3: { fontSize: "1.875rem", lineHeight: 1.2 },
  h4: { fontSize: "1.5rem", fontWeight: 600, lineHeight: 1.5 },
  h5: { fontSize: "1.25rem", fontWeight: 600 },
  h6: { fontSize: "1.125rem", fontWeight: 600 },
  subtitle1: { fontSize: "1.125rem" },
  subtitle2: { fontSize: "0.875rem", fontWeight: 500 },
  body1: { fontSize: "0.875rem" },
  body2: { fontSize: "0.875rem", fontWeight: 400 },
  caption: { fontSize: "0.75rem", fontWeight: 400 },
}

export const shape = {
  borderRadius: 8,
}

// Тени — простые строки
export const shadowsLight = [
  "none",
  "0px 4px 16px rgba(0,0,0,0.07)",
  "0px 8px 16px rgba(0,0,0,0.05)",
  ...Array(23).fill("none"),
]

export const shadowsDark = [
  "none",
  "0px 4px 16px rgba(0,0,0,0.7)",
  "0px 8px 16px rgba(0,0,0,0.8)",
  ...Array(23).fill("none"),
]

// Функция получения токенов темы — чистый объект без createTheme
export const getDesignTokens = (mode: PaletteMode) => {
  const isDark = mode === "dark"
  return {
    palette: {
      mode,
      primary: {
        light: brand[200],
        main: brand[400],
        dark: brand[700],
        contrastText: brand[50],
      },
      info: {
        light: brand[100],
        main: brand[300],
        dark: brand[600],
        contrastText: gray[50],
      },
      warning: {
        light: orange[300],
        main: orange[400],
        dark: orange[800],
      },
      error: {
        light: red[300],
        main: red[400],
        dark: red[800],
      },
      success: {
        light: green[300],
        main: green[400],
        dark: green[800],
      },
      grey: { ...gray },
      divider: isDark ? alpha(gray[700], 0.6) : alpha(gray[300], 0.4),
      background: {
        default: isDark ? gray[900] : "hsl(0, 0%, 99%)",
        paper: isDark ? "hsl(220, 30%, 7%)" : "hsl(220, 35%, 97%)",
      },
      text: {
        primary: isDark ? "hsl(0, 0%, 100%)" : gray[800],
        secondary: isDark ? gray[400] : gray[600],
      },
      action: {
        hover: isDark ? alpha(gray[600], 0.2) : alpha(gray[200], 0.2),
        selected: isDark ? alpha(gray[600], 0.3) : alpha(gray[200], 0.3),
      },
    },
    typography,
    shape,
    shadows: isDark ? shadowsDark : shadowsLight,
  }
}
// Объединяем light и dark тени в один объект shadows
export const shadows = {
  light: shadowsLight,
  dark: shadowsDark,
}

export const colorSchemes = {
  light: getDesignTokens("light"),
  dark: getDesignTokens("dark"),
}
